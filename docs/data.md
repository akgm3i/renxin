...
### 2.0. リレーションシップ概要
- `players` (1) --- (*) `game_participants` (中間テーブル)
- `games` (1) --- (*) `game_participants` (中間テーブル)
- `games` (1) --- (*) `rounds`
- `game_participants` (1) --- (*) `throws`
- `rounds` (1) --- (*) `throws`
- `rounds` (1) --- (*) `zero_one_round_states`
- `game_participants` (1) --- (*) `zero_one_round_states`
- `rounds` (1) --- (*) `cricket_marks`
- `game_participants` (1) --- (*) `cricket_marks`
- `rounds` (1) --- (*) `cricket_scores`
- `game_participants` (1) --- (*) `cricket_scores`

### 2.0.1. インデックス戦略
パフォーマンス向上のため、以下のカラムにインデックスを適用することを推奨する。
- `players`: `name` (検索、ソート用)
- `games`: `started_at`, `game_type`, `status` (ゲーム履歴のフィルタリング、ソート用)
- `game_participants`: `game_id`, `player_id` (外部キー結合用)
- `rounds`: `game_id`, `round_number` (ラウンド検索用)
- `throws`: `round_id`, `participant_id`, `created_at` (投擲結果の検索、時系列分析用)
- `zero_one_round_states`: `round_id`, `participant_id`, `created_at` (01ゲームの状態検索用)
- `cricket_marks`: `round_id`, `participant_id`, `target_number`, `created_at` (Cricketマークの検索、集計用)
- `cricket_scores`: `round_id`, `participant_id`, `created_at` (Cricketスコアの検索用)

特に、`created_at` カラムは、時系列でのデータ取得や分析に頻繁に利用されるため、多くのテーブルでインデックスの対象とする。
