...
- `npm run build` が失敗しており、現状は「常時 Green」ではない（`Button.test.tsx` の未使用 import による TypeScript エラー）。
...
1. スコープ確定済みの4機能（スコア入力・記録・動画再生・観戦）を Phase 1-3 から逆算し、完了条件を再定義する。
2. Raspberry Pi + 単眼カメラ前提で、録画／再生／配信の性能上限（解像度・fps・同時処理）を先に実測する。
3. 01ゲーム先行でドメインロジックを TDD で実装し、続いて Cricket を追加する。
4. DARTSLIVEレーティング仕様（80% STATS, 直近30ゲーム加重, 換算表, 更新条件）を `docs/spec.md` と `docs/data.md` に明文化する。
5. 雛形（画面遷移の骨組み・基盤テスト・IPC疎通）が揃った時点で一度 `main` にマージし、以降は機能単位でブランチを切って実装を進める。
