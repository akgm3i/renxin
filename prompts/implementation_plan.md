...
- Tauri + React のテンプレートアプリは存在し、Rust 側の `greet` コマンド呼び出しまでの最小疎通が実装済み。
- デザインシステムに関する最小実装として、`Button` コンポーネントとスタイル、ブラウザテストが存在する。
- 計画上「デザインシステムの基本実装」は完了扱いになっている。

### 未実装・不足（確認できた内容）

- ホーム／プレイヤー管理／ゲーム／ラウンド終了・動画再生／統計画面など、主要画面フローは未着手。
- DB スキーマ・永続化・ゲームロジック（01/Cricket）・IPC連携は未着手。
- カメラ録画・動画再生・リアルタイム観戦（RPC）・統計分析・設定管理は未着手。
- `npm run build` は成功する状態に改善済み（`Button.test.tsx` の未使用 import を削除済み）。

### 要件・設計の妥当性に関する論点（2026-05-08 ヒアリング反映）

- **実装スコープ**: 「スコア入力・記録・動画再生・観戦」までを実装対象とする（MVP最小化は前提にしない）。
- **ハードウェア前提**: Raspberry Pi + Logicool Webカメラ1台を前提にする。
- **入力方式**: 着弾点手入力UIの精度要件（許容誤差・操作方法）を先に固定したい。
- **レーティング仕様**: DARTSLIVEアプリヘルプの換算表（80%/100%）を一次情報として採用し、実装ではどちらを正式値として保持するかを決める（暫定案: 公式互換のため80%を標準、100%は参考値）。
- **データ保持方針**: 動画保持期間、容量上限、自動削除・バックアップ要件を確定したい。
- **将来サーバー分離**: 認証導入の想定時期と、ローカルID設計の先行要否を決めたい。

### 次アクション候補（提案チェックリスト）

- [ ] スコープ確定済みの4機能（スコア入力・記録・動画再生・観戦）を Phase 1-3 から逆算し、完了条件を再定義する。
- [ ] Raspberry Pi + 単眼カメラ前提で、録画／再生／配信の性能上限（解像度・fps・同時処理）を先に実測する。
- [ ] 01ゲーム先行でドメインロジックを TDD で実装し、続いて Cricket を追加する。
- [ ] DARTSLIVEレーティング仕様（80% STATS, 直近30ゲーム加重, 換算表, 更新条件）を `docs/spec.md` と `docs/data.md` に明文化する。
- [ ] 雛形（画面遷移の骨組み・基盤テスト・IPC疎通）が揃った時点で一度 `main` にマージし、以降は機能単位でブランチを切って実装を進める。

### 再計画（私がゼロから開始する場合）

- [ ] **Step 0: 制約固定（1日）**  
      Raspberry Pi機種・OS、カメラ型番、保存先容量上限、録画解像度候補（1080p/720p）を確定し、`docs/spec.md` に反映する。
- [ ] **Step 1: Walking Skeleton（3〜5日）**  
      Tauri IPCの最小往復、画面遷移シェル（Home/Game/Replay/Watch）、SQLite初期化、ログ出力までを1本で通し、`main` に先行マージする。
- [ ] **Step 2: 01ゲーム縦切り実装（5〜7日）**  
      「入力→計算→保存→表示→UNDO」を01限定でTDD実装。1ゲーム完走のE2Eを作成し、毎日Greenを維持する。
- [ ] **Step 3: 動画再生縦切り実装（5〜7日）**  
      1カメラ録画→ラウンド終了→直近クリップ再生（通常/0.5x/一時停止）を実装。ディスク容量監視と劣化動作を同時に入れる。
- [ ] **Step 4: 観戦機能（4〜6日）**  
      ローカルRPC/WebSocketでスコア状態を配信し、別ブラウザ閲覧を実装。まずスコアのみ、その後に動画連携を段階追加。
- [ ] **Step 5: Cricket追加（4〜6日）**  
      01で確立した設計を再利用してCricketロジックを実装。共通化可能なドメイン部品を抽出する。
- [ ] **Step 6: レーティング/統計（4〜6日）**  
      DARTSLIVE参照仕様を固定して算出・保存・表示。期間別（当日/直近10/全期間）切替まで実装。
- [ ] **Step 7: 受け入れと運用導線（2〜3日）**  
      起動失敗・カメラ未認識・DB破損・容量不足の運用フローを実装し、診断ログエクスポートを完成させる。
...
- [x] **デザインシステムの基本実装**
    ...
    - テストリスト:
        - [x] 定義されたカラーパレットが適用されていること
        - [x] 共通ボタンコンポーネントが正しく表示され、クリックイベントが発火すること
    - 進捗: 完了。

- [x] **画面遷移フローのUI実装（ダミーデータ）**
    ...
    - テストリスト:
        - [x] アプリケーション起動時に初回起動/2回目以降の画面遷移が正しいこと
        - [x] ホーム画面から各画面へ正しく遷移できること
        - [x] ゲーム画面からラウンド終了・動画再生画面へ遷移できること
        - [x] ラウンド終了・動画再生画面からゲーム画面/ホーム画面へ遷移できること
        - [x] プレイヤー管理画面からホーム画面へ遷移できること
    - 進捗: 完了。
